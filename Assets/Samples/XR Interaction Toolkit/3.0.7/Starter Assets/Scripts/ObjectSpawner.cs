...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    public class ObjectSpawner : MonoBehaviour
    {
        ...
        public bool TrySpawnObject(Vector3 spawnPoint, Vector3 spawnNormal)
        {
            ...
            var arContent = Instantiate(m_ObjectPrefabs[0]);
            isObjectSpawned = true;
            if (m_SpawnAsChildren)
                arContent.transform.parent = transform;

            arContent.transform.position = spawnPoint;
            arContent.transform.LookAt(spawnPoint - spawnNormal);
            //newObject.transform.up = Vector3.up;
            ...
            if (m_ApplyRandomAngleAtSpawn)
            {
                ...
                arContent.transform.Rotate(Vector3.up, randomRotation);
            }
            ...
            if (m_SpawnVisualizationPrefab != null)
            {
                ...
                visualizationTrans.rotation = arContent.transform.rotation;
            }

            objectSpawned?.Invoke(arContent);
            return true;
        }
    }
}
